    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data["success"]:
                ...
                raise requests.RequestException(f'data["succes"]=false @ catalog')
        else:
            raise requests.RequestException(f"status={response.status_code} @ catalog")
        ...
        if response.status_code == 200:
            ...
            if data["success"]:
                ...
                dataset = (
                    ...
                    ((t.get("name") or "") for t in (ds.get("tags") or [])),  # "tags":
                    (
                        (t.get("description") or "") for t in (ds.get("groups") or [])
                    ),  # "groups":
                )
                ...
                raise requests.RequestException(f'data["succes"]=false @ id={id}')
        else:
            raise requests.RequestException(f"status={response.status_code} @ id={id}")
    except requests.RequestException as e:
        print(f"error: {e}")
